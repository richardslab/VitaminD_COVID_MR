...
found `r nrow(significant_snps)` signifiant and common SNPs.
...
map_outcome_data <- function(x) {
  ...
  print(glue("{nrow(outcome_data)} exposure SNPs were found in the outcome file {params$outcome_file}"))
```


Make sure that the outcome data validates as a gwas
```{r Validate outcome data}
validations <- summary(confront(outcome_data, gwas_rules))
...
subset(pruned_combined_snps, rsid %notin% guillaume$rsid)
subset(guillaume, rsid %notin% pruned_combined_snps$rsid)


setdiff(pruned_combined_snps$rsid, guillaume$rsids)
setdiff(guillaume$rsid, pruned_combined_snps$rsid)

rsids_differences=rbind(data.frame(subset(guillaume,rsid %in% setdiff(guillaume$rsid, pruned_combined_snps$rsid)),set="Y"),
                        data.frame(subset(pruned_combined_snps, rsid %in% setdiff(pruned_combined_snps$rsid, guillaume$rsid), select=c("rsid","CHR")), set="G"))

subset(proxies, rsid %in% rsids_differences$rsid)


ld_pairs_raw <- get_LD_pairs(rsids_differences, population = params$pop, token = params$LDLink_token)
ld_pairs_raw%<>%subset(value>0.5 & variable!=RS_number)

merge(merge(ld_pairs_raw,rsids_differences, by.x=c("RS_number","CHR"), by.y=c("rsid","CHR")), rsids_differences, by.x=c("variable","CHR"), by.y=c("rsid","CHR"))

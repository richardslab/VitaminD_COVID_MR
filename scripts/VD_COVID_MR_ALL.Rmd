...
proxies_with_exposure <- choose_best_proxies(exposure_and_proxies)
```

```{r eval=FALSE}
exposure_and_proxies%>%subset(as.character(query_rsid)==missing)
...
# missing="rs10832289" # I chose a different LD linked snp rs10832310 but it's chimeric....:-(
...
subset(pruned_combined_snps, rsid==missing)
...
subset(ld_pairs,(RS_number==missing | value==missing ) & value >0.05 & RS_number!=variable)$variable %in% pruned_combined_snps$rsid
subset(pruned_combined_snps,rsid %in% subset(ld_pairs,(RS_number==missing | value==missing ) & value >0.05 & RS_number!=variable)$variable)
